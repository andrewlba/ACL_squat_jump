...
def jumpSquatPreProcess(data):
    ...
    '''
    # Drop unused columns
    data = data.drop(['ground_force1_py','ground_torque1_x','ground_torque1_y','ground_torque1_z','ground_force2_py','ground_torque2_x','ground_torque2_y','ground_torque2_z'], axis = 1)
    ...
    data['ground_force_totaly'] = data['ground_force1_vy'] + data['ground_force2_vy']
    ...
    while pass_val == 0:
        ...
        for n in range(len(in_air_bool)-1):
            if in_air_bool[n] == False and in_air_bool[n + 1] == True:
                ...
                pass
        if (len(start_in_air) == 4 or len(start_in_air) == 3) and len(end_in_air) == 3:
            pass_val = 1
        else:
            threshold += 5
    ...
    for in_air in [[contact_1[1]+1,contact_2[0]-1],[contact_2[1]+1,contact_3[0]-1],[contact_3[1]+1,contact_4[0]-1]]:
    ...
    for n in range(len(data['time'])):
        ground_force1_angle.append((np.arctan(data['ground_force1_vy'][n]/np.sqrt(xz1[n]))) * 180/np.pi)
        ground_force2_angle.append((np.arctan(data['ground_force2_vy'][n]/np.sqrt(xz2[n]))) * 180/np.pi)
    ...
    for n in range(len(data['ground_force_totaly'])-1):
        force_prime.append((data['ground_force_totaly'][n + 1] - data['ground_force_totaly'][n])/dt)
    ...
    # Find Subject Weight
    # Weight_bool-array of boolean where subject is mostly static and not in the air
    weight_bool = []
    ...
    # Static period function finds largest static period's indexes in a given range
    ...
    # Find static periods in second, third contact, and fourth contact (if needed)
    start_index_c234 = []
    end_index_c234 = []
    ...
    # Solve for 1st event start , start & stop of eccentric, start & stop of concentric
    ...
    # Solve for 1st event stop, landing start stop, 2nd event start, start & stop of eccentric, start & stop of concentric
    ...
    # Solve for 2nd event stop, landing start stop, 3rd event start, start & stop of eccentric, start & stop of concentric
    ...
    index_pd = {'Jump 1 Start': [evt1_s, ecc1_s, con1_s, ecc1_s, contact_2[0]],
             'Jump 1 End': [evt1_e, con1_s, contact_1[1], contact_1[1], evt1_e],
             'Jump 2 Start': [evt2_s, ecc2_s, con2_s, ecc2_s, contact_3[0]],
             'Jump 2 End': [evt2_e, con2_s, contact_2[1], contact_2[1], evt2_e],
             'Jump 3 Start': [evt3_s, ecc3_s, con3_s, ecc3_s, contact_4[0]],
             'Jump 3 End': [evt3_e, con3_s, contact_3[1], contact_3[1], evt3_e]}
    index_pd = pd.DataFrame(index_pd, index = ['Event', 'Eccentric Phase', 'Concentric Phase', 'Jump Phase', 'Landing Phase'])
